...
public class ModelWrapper implements IRetexturableModel, IModelCustomData, IModelSimpleProperties, IAnimatedModel
{
    ...
    public IBakedModel bake(IModelState state, VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter)
    {
        ...
        for (Map.Entry<String, ResourceLocation> e : textures.entrySet())
        {
            ...
            {
                FMLLog.severe("unresolved texture '%s' for opengex model '%s'", e.getValue().getResourcePath(), modelLocation);
    ...
    public ModelWrapper process(ImmutableMap<String, String> data)
    {
        if (data.containsKey("mesh"))
        {
            ...
            } else if (e.isJsonArray())
            {
                ...
                for (JsonElement s : e.getAsJsonArray())
                {
                    ...
                    {
                        FMLLog.severe("unknown mesh definition '%s' in array for opengex model '%s'", s.toString(), modelLocation);
                        return this;
                    }
                }
            ...
            {
                FMLLog.severe("unknown mesh definition '%s' for opengex model '%s'", e.toString(), modelLocation);
                return this;
            }
        }
        if (data.containsKey("key"))
        {
            ...
            {
                FMLLog.severe("unknown keyframe definition '%s' for opengex model '%s'", e.toString(), modelLocation);
                return this;
            }
        }
        return this;
